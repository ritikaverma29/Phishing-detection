...
class ChatInterface(Frame):
    ...
    # Options
        ...
        other.add_command(label="Enable/Disable voice output", command=self.vout)
       ...
       # color_theme.add_command(label='Mkbhd',command=self.MKBHD)
        ...
        self.send_button = Button(self.send_button_frame, text="Send", width=5, relief=GROOVE, bg='white',
                                  bd=1, command=lambda: self.send_message_in(None), activebackground="#FFFFFF",
                                  activeforeground="#000000")
        self.send_button.pack(side=LEFT, ipady=8)
        self.master.bind("<Return>", self.send_message_in)
    ...
    def send_message_in(self,message):
        if self.flag==0:
            self.send_message_insert2(None)
        else:
    ...
    def send_message_insert2(self, message):
        ...
        if ("http" in user_input) or ("www" in user_input) or (".com" in user_input):
            ...
            time.sleep(0)
            def ph(self,user_input):
                out = pred.phishing(user_input)
                pr="Bot : " + out + "\n"
                self.text_box.configure(state=NORMAL)
                self.text_box.insert(END, pr)
                self.text_box.configure(state=DISABLED)
                ...
                self.entry_field.delete(0,END)
                time.sleep(0)
            t = threading.Thread(target = ph,args=(self,user_input))
            t.start()
        else:
